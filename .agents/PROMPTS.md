...
> 汎用的なコンポーネントについてはtemplate-parts/ディレクトリにわかりやすいファイル名を作成して格納するようにして。また、固定ページ用テンプレートは、page-hogehoge.phpという名前になっていますが、接頭辞のpage-を削除して、templates/ディレクトリに移動させて、固定ページ編集画面からテンプレートを選ぶことで紐付けする仕様に変更してください
