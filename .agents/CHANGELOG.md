...
- 投稿一覧カードを `template-parts/post-list-item.php` に集約し、ブログ一覧とアーカイブで再利用。
- 固定ページ用カスタムテンプレートを `templates/` に移し、管理画面で明示的に選択する構成へ変更。
- 移動後の Job Details テンプレート判定、ディレクトリ運用ドキュメント、指示ログを更新。
