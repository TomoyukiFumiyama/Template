...
## 10. データベース操作方針（Database Policy）

- データベース操作は、WordPress API で実現できる処理を直接 SQL で実装しない。WordPress API を利用する方が安全性と互換性を保ちやすい。
- WordPress API では実現できず、どうしても SQL を記述する場合は、WordPress 公式推奨の `$wpdb->prepare()` を使用して値をプレースホルダーへ渡すこと。
